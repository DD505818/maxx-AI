"""Ensemble alpha generation engine."""
...
class AlphaEngine:
    """Blend momentum, mean-reversion, breakout, and sentiment."""

    def __init__(self, symbols: List[str] = None):
        symbols = symbols or ["BTCUSD", "ETHUSD", "EURUSD"]
        self.prices: Dict[str, deque] = {sym: deque(maxlen=200) for sym in symbols}

    def update(self, symbol: str, price: float) -> None:
        self.prices[symbol].append(price)

    def score(self, symbol: str) -> float:
        p = list(self.prices[symbol])
        if len(p) < 50:
            return 0.0
    ...
    def size(self, price: float, balance: float, alpha: float, vix: float = 18) -> float:
        recent = list(self.prices["BTCUSD"])[-50:]
        vol = np.std(recent) if recent else 1
